...
suspend fun Application.repoRoomGetLastSessionSkateByDate() =
    roomSessionDb().getLastSessionSkateByDate()

fun Application.repoRoomGetHistorySessionSkate() = roomSessionDb().getHistorySessionSkateFlow()
fun Application.repoRoomCountHistorySessionSkate() =
    roomSessionDb().getHistoryCountSessionSkateFlow()
...
suspend fun Application.repoRoomGetIdSession(idSession: Long) =
    roomSessionDb().getSessionSkateId(idSession)

fun Application.repoRoomGetIdSessionFlow(idSession: Long) =
    roomSessionDb().getSessionSkateIdFlow(idSession)

suspend fun Application.repoRoomInsertSession(roomSession: RoomSession): OperationResult<Long> {
    return try {
        val id = roomSessionDb().insertSession(roomSession)
        if (id == -1L) OperationResult.Error("No se pudo guardar la sesión correctamente.")
        else OperationResult.Success(id)
    } catch (e: Exception) {
